...
class Blurhash
{
    public static function encode(array $image, int $components_x = 4, int $components_y = 4, bool $linear = \false) : string
    {
        ...
        if (!$linear) {
            ...
            for ($y = 0; $y < $height; $y++) {
                ...
                for ($x = 0; $x < $width; $x++) {
                    $pixel = $image[$y][$x];
                    $line[] = [\Lazysizes\Vendor\kornrunner\Blurhash\Color::toLinear($pixel[0]), \Lazysizes\Vendor\kornrunner\Blurhash\Color::toLinear($pixel[1]), \Lazysizes\Vendor\kornrunner\Blurhash\Color::toLinear($pixel[2])];
                }
                $image_linear[] = $line;
            }
        }
            ...
            for ($x = 0; $x < $components_x; $x++) {
                ...
                for ($i = 0; $i < $width; $i++) {
                    for ($j = 0; $j < $height; $j++) {
                        ...
                        $basis = $normalisation * \cos(\M_PI * $i * $x / $width) * \cos(\M_PI * $j * $y / $height);
                        $r += $basis * $color[0];
                        $g += $basis * $color[1];
                        $b += $basis * $color[2];
                    }
        ...
        $ac_values = [];
        foreach ($components as $component) {
            $ac_values[] = \Lazysizes\Vendor\kornrunner\Blurhash\AC::encode($component, $ac_component_norm_factor);
        }
        $blurhash = \Lazysizes\Vendor\kornrunner\Blurhash\Base83::encode($components_x - 1 + ($components_y - 1) * 9, 1);
        $blurhash .= \Lazysizes\Vendor\kornrunner\Blurhash\Base83::encode($quant_max_ac_component, 1);
        $blurhash .= \Lazysizes\Vendor\kornrunner\Blurhash\Base83::encode($dc_value, 4);
        foreach ($ac_values as $ac_value) {
            $blurhash .= \Lazysizes\Vendor\kornrunner\Blurhash\Base83::encode((int) $ac_value, 2);
        }
        return $blurhash;
    }
    public static function decode(string $blurhash, int $width, int $height, float $punch = 1.0, bool $linear = \false) : array
    {
        ...
        $colors = [\Lazysizes\Vendor\kornrunner\Blurhash\DC::decode(\Lazysizes\Vendor\kornrunner\Blurhash\Base83::decode(\substr($blurhash, 2, 4)))];
        $quant_max_ac_component = \Lazysizes\Vendor\kornrunner\Blurhash\Base83::decode($blurhash[1]);
        $max_value = ($quant_max_ac_component + 1) / 166;
        for ($i = 1; $i < $size_x * $size_y; $i++) {
            $value = \Lazysizes\Vendor\kornrunner\Blurhash\Base83::decode(\substr($blurhash, 4 + $i * 2, 2));
            $colors[$i] = \Lazysizes\Vendor\kornrunner\Blurhash\AC::decode($value, $max_value * $punch);
        }
        $pixels = [];
        for ($y = 0; $y < $height; $y++) {
            $row = [];
            for ($x = 0; $x < $width; $x++) {
                ...
                $row[] = $linear ? [$r, $g, $b] : [\Lazysizes\Vendor\kornrunner\Blurhash\Color::toSRGB($r), \Lazysizes\Vendor\kornrunner\Blurhash\Color::toSRGB($g), \Lazysizes\Vendor\kornrunner\Blurhash\Color::toSRGB($b)];
            }
            $pixels[] = $row;
        }
        return $pixels;
